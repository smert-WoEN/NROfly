...
image = cv2.imread("../FifthTask/1.png", cv2.IMREAD_COLOR)
...
while True:
    img = cv2.GaussianBlur(image, (25, 25), cv2.BORDER_DEFAULT)
    #img = image.copy()
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    ...
    hsvMin = np.array((0, 0, 0), np.uint8)
    ...
    if len(counter) > 1:
        print(cv2.boundingRect(counter[1]))
